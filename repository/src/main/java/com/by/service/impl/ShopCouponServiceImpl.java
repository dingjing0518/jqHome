...
public class ShopCouponServiceImpl implements ShopCouponService {
	...
	public ShopCoupon cachePut(ShopCoupon coupon) {
		return coupon;
	}
	...
	public ShopCoupon update(ShopCoupon coupon) {
		...
		s.setBeginTime(coupon.getBeginTime());
		s.setEndTime(coupon.getEndTime());
		s.setCouponEndTime(coupon.getCouponEndTime());
		return s;
	}
	...
	public ShopCoupon refresh(int id) {
		return repository.findOne(id);
	}
	...
	@Override
	public Page<ShopCoupon> findAllByValidAndDateBetweenAndNameLike(ValidEnum valid, Calendar calendar, String name,
			Pageable pageable) {
		return repository.findAllByValidAndDateBetweenAndNameLike(ValidEnum.VALID, name, Calendar.getInstance(),
				pageable);
	}
