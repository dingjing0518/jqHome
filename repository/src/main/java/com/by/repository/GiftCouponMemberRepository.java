...
public interface GiftCouponMemberRepository extends PagingAndSortingRepository<GiftCouponMember, Long> {
    ...
    @Query("select gc from GiftCouponMember gc where gc.usedTime is null and gc.member=:member and gc.coupon.valid=:valid and ((:today<gc.coupon.couponEndTime) or (gc.coupon.couponEndTime is null)) order by gc.coupon.couponEndTime")
    List<GiftCouponMember> findByMemberAndValid(@Param("member") Member member, @Param("valid") ValidEnum valid, @Param("today") Calendar today);
    ...
    GiftCouponMember findByCodeAndMember(String code,Member member);
}
